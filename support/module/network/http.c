...
HTTP_PARSE_RESULT *http_parse_querry_str(char *querry_str)
{
	...
	// type of querry
	if(strncmp(querry_str,		"GET", 3)==0)		{ result.type = HTTP_QUERRY_TYPE_GET;		pt_querry_str+=3; }
	else if(strncmp(querry_str,	"HEAD", 4)==0)		{ result.type = HTTP_QUERRY_TYPE_HEAD;		pt_querry_str+=4; }
	else if(strncmp(querry_str,	"POST", 4)==0)		{ result.type = HTTP_QUERRY_TYPE_POST;		pt_querry_str+=4; }
	else if(strncmp(querry_str,	"PUT", 3)==0)		{ result.type = HTTP_QUERRY_TYPE_PUT;		pt_querry_str+=3; }
	else if(strncmp(querry_str,	"DELETE", 6)==0)	{ result.type = HTTP_QUERRY_TYPE_DELETE;	pt_querry_str+=6; }
#ifdef HTTP_COMPLETE_PROTO
	else if(strncmp(querry_str,	"CONNECT", 7)==0)	{ result.type = HTTP_QUERRY_TYPE_CONNECT;	pt_querry_str+=7; }
	else if(strncmp(querry_str,	"OPTIONS", 7)==0)	{ result.type = HTTP_QUERRY_TYPE_OPTIONS;	pt_querry_str+=7; }
	else if(strncmp(querry_str,	"TRACE", 5)==0)		{ result.type = HTTP_QUERRY_TYPE_TRACE;		pt_querry_str+=5; }
#endif
	else return &result;
	
	// check the space after
	if(*pt_querry_str != ' ') return &result;
	...
	if(pt_end_path==NULL && pt_end_line<=pt_end_path) return &result;
