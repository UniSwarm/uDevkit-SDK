...
void ScreenWidget::writeData(const uint16_t *pix, size_t size)
{
    QPainter painter(&_pixmap);
...
QColor ScreenWidget::fromData(uint16_t pixValue) const
{
    QColor color;
    switch (_colorModde)
    {
        case ColorModeMono:
            ...
            break;
        case ColorMode565:
            color = QColor((pixValue & 0xF800) >> 8, (pixValue & 0x07E0) >> 3, (pixValue & 0x001F) << 3);
            break;
        default:
            break;
    }
